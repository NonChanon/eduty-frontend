  ...
  return (
    <div className="w-full p-5 border-[#F9F9F9] border-2 rounded">
      ...
      <div className="relative overflow-x-auto mt-5">
                ...
                <td className="px-6 py-4">
                  <button
                    ...
                    className={twClassName(
                      "status",
                      lot.sendRdStatus.toLowerCase()
                    )}
                  >
                    {lot.sendRdStatus === "WAIT_FOR_APPROVE"
                      ? "In Progess"
                      : lot.sendRdStatus.toLowerCase().replaceAll("_", " ")}
                  </button>
                </td>
        ...
        <PagingTab
          handlePaging={handleDisplayPerDate}
          totalPage={props.paging.totalPage as unknown as number}
        />
      </div>
    </div>
  );
